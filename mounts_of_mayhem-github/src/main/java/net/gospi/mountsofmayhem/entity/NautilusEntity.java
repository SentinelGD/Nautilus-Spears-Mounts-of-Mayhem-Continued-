...
public class NautilusEntity extends TamableAnimal {
	...
	public void tick() {
		...
		// Применяем внешнее движение если оно есть (после travel)
	...
	public void travel(Vec3 dir) {
		if (this.isVehicle()) {
			...
			if (passenger != null) {
				...
				// Управление в воде
				if (this.isInWater()) {
					// Sprint al moverse hacia adelante (no se puede detectar tecla
					// de correr mientras monta — Minecraft la bloquea en Player.aiStep)
				...
				// Aplicar dash boost si está activo (solo en agua)
				if (this.isInWater() && this.dashTicks > 0 && !this.dashBoost.equals(Vec3.ZERO)) {
					...
					// Cancelar dash al salir del agua — evita rebote al re-entrar
					this.dashTicks = 0;
					this.dashBoost = Vec3.ZERO;
				}
